...
async function loadFromDb(): Promise<CompressionConfig> {
  // SpacetimeDB build: pull all settings, filter to compression_* in JS.
  // Cheap because the settings table is small (<50 rows in practice).
  const rows = db.settings
    .getAll()
    .filter((s: { key: string }) => s.key.startsWith("compression_"));
  const map = new Map<string, string | null>();
  for (const r of rows) map.set(r.key, r.value ?? null);
